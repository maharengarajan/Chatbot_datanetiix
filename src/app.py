...
def client():
    try:
        ...
        message = welcome_messages.get(
            client_type, "Invalid option. Please choose a valid option."
        )
...
def new_client_details():
    try:
        ...
        if not is_valid_email(email):
            return jsonify(
                {"message": "Please enter a valid email address.", "code": 400}
            )
...
@app.route(
    "/chatbot/new_client/user_details/industries/verticals/requirement",
    methods=["POST"],
)
def requirement():
    try:
...
@app.route(
    "/chatbot/new_client/user_details/industries/verticals/requirement/known_source",
    methods=["POST"],
)
def known_source():
    try:
        ...
        if selected_option in known_sources:
            ...
            return jsonify(
                {"selected_known_source": selected_known_source, "code": 200}
            )
        else:
            return jsonify({"message": "Please choose a valid option.", "code": 400})
    except Exception as e:
...
@app.route(
    "/chatbot/existing_client_details/verticals/issue_escalation", methods=["POST"]
)
...
@app.route(
    "/chatbot/existing_client_details/verticals/issue_escalation/issue_type",
    methods=["POST"],
)
def issue_type():
    try:
...
def category():
    try:
        ...
        if user_type in category_type:
            ...
            return jsonify(
                {"user_type": selected_category_type, "row_id": row_id, "code": 200}
            )
        else:
            return jsonify({"message": "Please choose a valid option.", "code": 400})
    except Exception as e:
...
@app.route(
    "/chatbot/job_seeker_details/category/verticals/interview_avail", methods=["POST"]
)
...
@app.route(
    "/chatbot/job_seeker_details/category/verticals/interview_avail/date_of_interview",
    methods=["POST"],
)
...
@app.route(
    "/chatbot/job_seeker_details/category/verticals/interview_avail/date_of_interview/notice_period",
    methods=["POST"],
)
def notice_period():
    try:
        ...
        if joining_date in notice_period_options:
            ...
            return jsonify(
                {"error": "Invalid input. Please select a valid option.", "code": 400}
            )
    except Exception as e:
        return jsonify(
            {"message": "Internal server error.", "error": str(e), "code": 500}
