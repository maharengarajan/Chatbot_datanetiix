...
def get_location():
    ...
    response = requests.get(
        f"https://ipapi.co/{ip_address}/city/"
    ).text  # Up-to 30000 free requests per month (1000 in 24 hours)
    city = response
    return city
...
def choose_industries_option():
    ...
    while True:
        ...
        options = [
            opt.strip() for opt in option.split(",") if opt.strip() in valid_options
        ]
        if options:
...
def issue_type():
    while True:
        ...
        if response == "1":
            print(
                "Thank you. We have saved your issue and will contact you as soon as possible."
            )
            return "normal"  # Return as a string
        elif response == "2":
            print(
                "Thank you. We have saved your issue as urgent and will contact you immediately."
            )
...
def category():
    while True:
        user_type = input(
            "Are you an experienced or fresher? (Enter 1 for experienced, 2 for fresher): "
        )
        if user_type == "1":
...
def interview_available_check():
    while True:
        interview_avail = input(
            "Are you available for an interview? (1 for yes, 2 for no): "
        )
...
def date_of_interview():
    while True:
        try:
            time_avail = input(
                "What is your time availability for an interview? (Please enter a date in dd/mm/yyyy format): "
            )
...
def notice_period():
    while True:
        joining_date = input(
            "When can you join? (1 for 30 days/2 for 60 days/3 for 90 days): "
        )
        if joining_date == "1":
...
client_type = input(
    "Can you please let me know if you are a? (Enter 1 for new client, 2 for existing client, 3 for job seeker, 4 to exit): "
)
